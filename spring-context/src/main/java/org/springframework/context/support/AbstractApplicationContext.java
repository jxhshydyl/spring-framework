...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			//注意，这里将会初始化 BeanFactory、
			// 加载 Bean：从配置文件中加载<bean> 并转化成 BeanDefinition
			// 注册 Bean：把BeanDefinition注册到 DefaultListableBeanFactory.beanDefinitionMap
			// 注册bean会判断是否可以覆盖bean，通过DefaultListableBeanFactory.allowBeanDefinitionOverriding属性值判断
			// 同一配置文件的bean不会覆盖直接抛出异常，只有不同配置文件的才会判断是否需要覆盖
			...
			try {
				 ...
				 * 并且所有的实现了 BeanFactoryPostProcessor 接口的 Bean 都已经初始化并且
				 * 其中的 postProcessBeanFactory(factory) 方法已经得到回调执行了。
				 * 而且 Spring 已经“手动”注册了一些特殊的 Bean，如 environment、systemProperties 等。
				 * 剩下的就是初始化 singleton beans 了，我们知道它们是单例的，如果没有设置懒加载，
				 * 那么 Spring 会在接下来初始化所有的 singleton beans。
				 */
	 ...
	 * @see #refreshBeanFactory()
	 * @see #getBeanFactory()
	 * @see ConfigurableListableBeanFactory 接口的实现类  只有DefaultListableBeanFactory
	 */
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// conversionService:为了统一调用Converter进行类型转换，spring为我们提供了一个ConversionService接口。
